...
export const ExpenseCategoryForm: React.FC = () => {
  ...
  useEffect(() => {
    if (isEdit && id) {
      const loadCategory = async () => {
        setFetching(true);
        try {
          const cat = await db.expenseCategories.getById(id);
          if (cat) {
            setForm({
              name: cat.name || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && id) {
        await db.expenseCategories.update(id, form);
      } else {
        await db.expenseCategories.create(form);
      }
      navigate('/expenses');
    } catch (err: any) {
  ...
  return (
    <PermissionGuard permission="Expenses.Create" fallback="ui">
        ...
        <form onSubmit={handleSubmit} className="glass border border-border rounded-2xl p-6 space-y-5 shadow-xl">
          ...
          <div className="space-y-1.5 text-xs">
            ...
            <input
              id="name"
              type="text"
              placeholder="E.g. Shop Rent, Toner Refill, Staff Wages"
              value={form.name}
          ...
          <div className="space-y-1.5 text-xs">
            ...
            <textarea
              id="description"
              placeholder="E.g. Details regarding leasing agreements, ink purchases, monthly allocations..."
