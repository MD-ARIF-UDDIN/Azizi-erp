...
export const SalesList: React.FC = () => {
  ...
  // Gov Fee / Service Cost form in Payment Modal
  const [payGovItemId, setPayGovItemId] = useState('all');
  const [payGovAmount, setPayGovAmount] = useState(0);
  const [payGovAccountId, setPayGovAccountId] = useState('');
  const [payGovDate, setPayGovDate] = useState(new Date().toISOString().split('T')[0]);
  const [payGovDesc, setPayGovDesc] = useState('');
  const [payGovSaving, setPayGovSaving] = useState(false);
  ...
  const handleOpenEditItems = async (saleId: string, preselectedItemId?: string) => {
    ...
    // 1. Instantly populate from already-loaded sales in memory for 0ms lag
    const existing = sales.find(s => s.id === saleId);
    if (existing) {
      setEditingSale(existing);
    ...
    // 2. Refresh detail smoothly in background
    try {
      ...
      if (detail) {
        ...
        if (preselectedItemId && items.some((i: any) => i.id === preselectedItemId)) {
          setSelectedServiceItemId(preselectedItemId);
        } else if (items.length > 0 && !selectedServiceItemId) {
          setSelectedServiceItemId(items[0].id);
        }
      }
    } catch (err) {
  ...
  const handleOpenPayModal = async (saleId: string) => {
    ...
    const existing = sales.find(s => s.id === saleId);
    if (existing) {
      setPayingSaleDetails(existing);
      const totalPaid = (existing.payments || []).reduce((sum: number, p: any) => sum + p.amount, 0);
  ...
  const [salesConfirmModal, setSalesConfirmModal] = useState<{
    isOpen: boolean;
    type: 'payment' | 'refund' | 'expense' | 'generic';
    title: string;
    subtitle?: string;
    amount?: number;
    confirmText?: string;
  ...
  const handleSubmitPayment = (e: React.FormEvent) => {
    ...
    const totalCollected = (payingSaleDetails?.payments || []).reduce((sum: number, p: any) => sum + (p.amount || 0), 0);
    const grandTotal = Number(payingSaleDetails?.grand_total) || 0;
    const due = Math.max(0, grandTotal - totalCollected);
    const isAdv = payAmount > due || totalCollected >= grandTotal;

    const details: ConfirmDetailItem[] = [
      {
        label: 'Invoice Reference',
        value: `#${payingSaleDetails?.invoice_no}`,
        badge: true,
        badgeColor: 'sky'
      },
      {
        label: 'Customer Name',
        value: payingSaleDetails?.customer?.name || 'Walk-in / Individual'
      },
      {
        label: 'Payment Mode',
        value: payMethod,
    ...
    if (payPersonName.trim()) {
      details.push({
        label: 'For Applicant / Member',
        value: `👤 ${payPersonName.trim()}`,
        badge: true,
        badgeColor: 'indigo'
      });
    }
    ...
    details.push({
      label: 'Outstanding Due Before',
      value: `${due.toFixed(2)} AED`
    });

    details.push({
      label: isAdv ? 'Advance Surplus Recorded' : 'Remaining Due After Payment',
      value: isAdv ? `+${(payAmount - due).toFixed(2)} AED` : `${Math.max(0, due - payAmount).toFixed(2)} AED`,
      highlight: true
    });
    ...
    setSalesConfirmModal({
      isOpen: true,
      type: 'payment',
      title: isAdv ? 'Confirm Advance / Additional Payment' : 'Confirm Payment Collection',
      subtitle: `Please verify payment allocation for invoice #${payingSaleDetails?.invoice_no}.`,
      amount: payAmount,
      confirmText: isAdv ? 'Confirm & Save Advance' : 'Confirm & Collect Payment',
      details,
      onConfirm: executeSubmitPayment
    });
  };

  const executeSubmitRefund = async () => {
    if (!payingSaleId || payAmount <= 0 || !payAccountId || !refundReason.trim()) return;
    setPaySaving(true);
    try {
  ...
  const handleSubmitRefund = (e: React.FormEvent) => {
    ...
    const details: ConfirmDetailItem[] = [
      ...
      {
        label: 'Customer Name',
        value: payingSaleDetails?.customer?.name || 'Customer'
      },
      {
        label: 'Payout From Account (Cash Out)',
        value: selectedAcc ? `${selectedAcc.name} (Current: ${Number(selectedAcc.balance).toFixed(2)} AED)` : 'Account',
        badge: true,
        badgeColor: 'rose'
      },
      {
      ...
      {
        label: 'Reason for Money Return',
        value: refundReason.trim(),
        highlight: true
      }
    ];
    ...
    setSalesConfirmModal({
      isOpen: true,
      type: 'refund',
      title: 'Confirm Return Money / Refund',
      subtitle: `Funds will be returned to client and recorded in the cash out ledger.`,
      amount: payAmount,
      confirmText: 'Confirm & Process Refund',
      warningMessage: 'This action will deduct cash/funds from the selected account and generate an official refund voucher.',
      details,
      onConfirm: executeSubmitRefund
    });
  };

  const executeLogGovFee = async () => {
    if (!payingSaleId || payGovAmount <= 0) return;
    setPayGovSaving(true);
    try {
      const selectedItem = payingSaleDetails?.items?.find((i: any) => i.id === payGovItemId) || payingSaleDetails?.items?.[0];
      const srvName = selectedItem?.service?.name || 'Service';
      const personSuffix = selectedItem?.person_name ? ` (${selectedItem.person_name})` : (payingSaleDetails?.person_name ? ` (${payingSaleDetails.person_name})` : '');
      const defaultDesc = `${srvName} Gov Fee${personSuffix}`;
      const desc = payGovDesc.trim() || defaultDesc;

      await db.sales.addServiceExpense({
        sale_id: payingSaleId,
        sale_item_id: selectedItem?.id || payingSaleId,
        amount: Number(payGovAmount),
        account_id: payGovAccountId || undefined,
        description: desc,
        expense_date: payGovDate || new Date().toISOString().split('T')[0]
      });

      const updatedSale = await db.sales.getById(payingSaleId);
      setPayingSaleDetails(updatedSale);
      if (editingSaleId && editingSaleId === payingSaleId) {
        setEditingSale(updatedSale);
        setEditingSaleItems(updatedSale?.items || []);
      }
      if (selectedSaleId && selectedSaleId === payingSaleId) {
      ...
      alert(err.message || 'Failed to record gov fee expense');
      console.error(err);
    } finally {
      setPayGovSaving(false);
    }
  };

  const handleLogGovFeeFromPayModal = (e: React.FormEvent) => {
    e.preventDefault();
    if (!payingSaleId || payGovAmount <= 0) return;
    if (!payGovAccountId) {
      alert('Paid From Card / Account is required.');
      return;
    }

    const selectedItem = payingSaleDetails?.items?.find((i: any) => i.id === payGovItemId) || payingSaleDetails?.items?.[0];
    const srvName = selectedItem?.service?.name || 'General Invoice Fee';
    const personSuffix = selectedItem?.person_name ? ` (${selectedItem.person_name})` : '';
    const selectedAcc = accounts.find(a => a.id === payGovAccountId);
    ...
    const details: ConfirmDetailItem[] = [
      ...
      {
        label: 'Linked Service Item',
        value: `${srvName}${personSuffix}`,
        badge: true,
        badgeColor: 'indigo'
      },
      {
        label: 'Paid From (Card / Drawer)',
        value: selectedAcc ? `${selectedAcc.type === 'card' ? '💳' : selectedAcc.type === 'bank' ? '🏦' : '💵'} ${selectedAcc.name} (Bal: ${Number(selectedAcc.balance).toFixed(2)} AED)` : 'Account',
        badge: true,
        badgeColor: 'amber'
      },
      {
        label: 'Fee Description',
        value: payGovDesc.trim() || `${srvName} Gov Fee${personSuffix}`
      },
      {
        label: 'Expense Date',
        value: payGovDate || new Date().toLocaleDateString()
      }
    ];
    ...
    setSalesConfirmModal({
      isOpen: true,
      type: 'expense',
      title: 'Confirm Government Fee / Card Expense',
      subtitle: `Record official government portal fee charged to company card.`,
      amount: payGovAmount,
      confirmText: 'Confirm & Record Fee',
      details,
      onConfirm: executeLogGovFee
    });
  };

  const handleDeleteGovFeeFromPayModal = async (expenseId: string) => {
    if (!payingSaleId || !window.confirm('Delete this gov fee expense? The account balance will be restored.')) return;
    setPayGovSaving(true);
    try {
      await db.sales.deleteServiceExpense(expenseId);
      const updatedSale = await db.sales.getById(payingSaleId);
      ...
      alert(err.message || 'Failed to delete gov fee expense');
      console.error(err);
    } finally {
      setPayGovSaving(false);
    }
    ...
    <PermissionGuard permission="Sales.View" fallback="ui">
      <div className="space-y-5 print:p-0 print:bg-white print:text-black">
          ...
          <div className="flex flex-col lg:flex-row gap-2.5 pt-1 border-t border-border/60">
            ...
            <div className="flex flex-wrap sm:flex-nowrap gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20 transition-all w-full sm:w-auto justify-center cursor-pointer shadow-2xs"
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold border border-border text-foreground bg-muted/60 hover:bg-muted transition-all w-full sm:w-auto justify-center cursor-pointer shadow-2xs"
                title="Print or Save Invoices as PDF"
              >
        ...
        <div className="w-full">
          ...
          {/* ── PRINT VIEW: INVOICES DIRECTORY STATEMENT (When exporting PDF without a single invoice modal open) ── */}
          {!selectedSaleDetails && (
            <div className="hidden print:block text-slate-900 bg-white p-0 font-sans text-xs">
              {/* Header */}
              <div className="flex items-center justify-between pb-3 border-b-2 border-slate-900 gap-4 mb-3">
                <div className="flex items-center gap-3">
                  <img src="/logo.png" alt="AZIZI Logo" className="w-14 h-14 object-contain shrink-0" />
                  <div>
                    <h1 className="text-base font-black text-[#000ba0] tracking-tight uppercase leading-none font-heading m-0">
                      AZIZI TYPING &amp; STAMP MAKING
                    </h1>
                    <p className="text-[9px] font-semibold text-slate-700 mt-0.5 mb-0">
                      Musaffah M37, Abu Dhabi • Tel: 0542797933 • Email: azizitypingbr@gmail.com
                    </p>
                  </div>
                </div>
                <div className="text-right border-l-2 border-slate-300 pl-4 space-y-0.5">
                  <div className="text-xs font-black uppercase tracking-wider text-slate-900 bg-slate-100 px-2 py-0.5 rounded inline-block border border-slate-300">
                    INVOICES &amp; SALES DIRECTORY STATEMENT
                  </div>
                  <div className="text-[10px] text-slate-700 font-medium">
                    <span>Generated: <strong>{new Date().toLocaleString()}</strong></span>
                    <span className="mx-1.5">•</span>
                    <span>Total Invoices: <strong>{filteredSales.length}</strong></span>
                  </div>
                </div>
              </div>
              ...
              {(() => {
                const totalBilled = filteredSales.reduce((sum, s) => sum + (s.grand_total || 0), 0);
                const totalPaid = filteredSales.reduce((sum, s) => sum + (s.payments || []).reduce((pSum: number, p: any) => pSum + (p.amount || 0), 0), 0);
                const totalDue = Math.max(0, totalBilled - totalPaid);

                return (
                  <div className="grid grid-cols-3 gap-2 mb-3">
                    <div className="border border-slate-300 bg-slate-50 p-2 rounded text-center">
                      <div className="text-[9px] uppercase font-bold text-slate-500">Total Billed</div>
                      <div className="text-sm font-black text-slate-900">{totalBilled.toFixed(2)} AED</div>
                    </div>
                    <div className="border border-emerald-300 bg-emerald-50/50 p-2 rounded text-center">
                      <div className="text-[9px] uppercase font-bold text-emerald-700">Total Collected</div>
                      <div className="text-sm font-black text-emerald-800">{totalPaid.toFixed(2)} AED</div>
                    </div>
                    <div className="border border-rose-300 bg-rose-50/50 p-2 rounded text-center">
                      <div className="text-[9px] uppercase font-bold text-rose-700">Total Outstanding Due</div>
                      <div className="text-sm font-black text-rose-800">{totalDue.toFixed(2)} AED</div>
                    </div>
                  </div>
                );
              })()}
              ...
              <table className="w-full text-left border-collapse border border-slate-300 text-[10px]">
                <thead>
                  <tr className="bg-[#000ba0] text-white font-bold uppercase tracking-wider text-[9px]">
                    <th className="px-2 py-1.5 text-center border border-slate-400 w-8">SL</th>
                    <th className="px-2 py-1.5 border border-slate-400 w-24">Invoice #</th>
                    <th className="px-2 py-1.5 border border-slate-400 w-20">Date</th>
                    <th className="px-2 py-1.5 border border-slate-400">Customer &amp; Members</th>
                    <th className="px-2 py-1.5 text-center border border-slate-400 w-20">Job Status</th>
                    <th className="px-2 py-1.5 text-center border border-slate-400 w-20">Payment</th>
                    <th className="px-2 py-1.5 text-right border border-slate-400 w-20">Total</th>
                    <th className="px-2 py-1.5 text-right border border-slate-400 w-20">Paid</th>
                    <th className="px-2 py-1.5 text-right border border-slate-400 w-20">Due</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredSales.map((s, idx) => {
                    const totalPaid = (s.payments || []).reduce((sum: number, p: any) => sum + (p.amount || 0), 0);
                    const due = Math.max(0, (s.grand_total || 0) - totalPaid);
                    ...
                    return (
                      <tr key={s.id} className="border-b border-slate-200">
                        <td className="px-2 py-1 text-center border-r border-slate-200 font-bold text-slate-500">{idx + 1}</td>
                        <td className="px-2 py-1 border-r border-slate-200 font-mono font-bold">#{s.invoice_no}</td>
                        <td className="px-2 py-1 border-r border-slate-200 text-slate-600">{new Date(s.created_at).toLocaleDateString()}</td>
                        <td className="px-2 py-1 border-r border-slate-200">
                          <div className="font-bold text-slate-900">{custName}</div>
                          {memberNames.length > 0 && (
                            <div className="text-[9px] text-slate-500 italic">👤 {memberNames.join(', ')}</div>
                          )}
                        </td>
                        <td className="px-2 py-1 text-center border-r border-slate-200 text-[9px] font-semibold">{s.order_status?.name || '—'}</td>
                        <td className="px-2 py-1 text-center border-r border-slate-200 text-[9px] font-bold">
                          {due <= 0 ? 'PAID' : totalPaid > 0 ? 'PARTIAL' : 'UNPAID'}
                        </td>
                        <td className="px-2 py-1 text-right border-r border-slate-200 font-mono font-bold">{s.grand_total.toFixed(2)}</td>
                        <td className="px-2 py-1 text-right border-r border-slate-200 font-mono text-emerald-700">{totalPaid.toFixed(2)}</td>
                        <td className="px-2 py-1 text-right font-mono font-bold text-rose-700">{due.toFixed(2)}</td>
                      </tr>
                    );
                  })}
                </tbody>
                <tfoot>
                  {(() => {
                    ...
                    return (
                      <tr className="bg-slate-100 font-black border-t-2 border-slate-400 text-[10px]">
                        <td colSpan={6} className="px-2 py-1.5 text-center uppercase tracking-wider border-r border-slate-300">
                          Grand Totals ({filteredSales.length} Invoices)
                        </td>
                        <td className="px-2 py-1.5 text-right font-mono border-r border-slate-300">{totalBilled.toFixed(2)}</td>
                        <td className="px-2 py-1.5 text-right font-mono text-emerald-800 border-r border-slate-300">{totalPaid.toFixed(2)}</td>
                        <td className="px-2 py-1.5 text-right font-mono text-rose-800">{totalDue.toFixed(2)}</td>
                      </tr>
                    );
                  })()}
                </tfoot>
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {editingSaleItems.length === 0 ? (
                          ...
                          editingSaleItems.map((item: any, idx: number) => {
                            ...
                            const itemExpTotal = itemExps.length > 0
                              ? itemExps.reduce((eSum: number, e: any) => eSum + (Number(e.amount) || 0), 0)
                              : (Number(item.service?.expense) || Number(item.expense || 0));
        ...
        {/* PAYMENT & ADVANCE / GOV FEE / REFUND MODAL */}
        {payModalOpen && payingSaleDetails && (() => {
          const items = payingSaleDetails.items || [];
          ...
          const totalGovFeesPaid = items.reduce((sum: number, i: any) => sum + (Number(i.expense) || 0), 0);
          const defaultExpectedGovFee = items.reduce((sum: number, i: any) => sum + ((Number(i.service?.expense) || 0) * (Number(i.quantity) || 1)), 0);
          const currentGovFeeCost = totalGovFeesPaid > 0 ? totalGovFeesPaid : defaultExpectedGovFee;
          const typingMarginProfit = grandTotal - currentGovFeeCost;

          const allItemExpenses: any[] = [];
          items.forEach((i: any) => {
            (i.expenses || []).forEach((exp: any) => {
              if (!allItemExpenses.some(x => x.id === exp.id)) {
                allItemExpenses.push({
                  ...exp,
                  itemService: i.service?.name,
                  itemPerson: i.person_name
                });
              }
            });
          });
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm print:hidden">
              <div className="glass border border-border rounded-2xl p-6 sm:p-7 w-full max-w-3xl sm:max-w-4xl bg-white shadow-2xl relative max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex items-center gap-2 mb-4">
                  <CreditCard size={18} className={payModalTab === 'refund' ? 'text-rose-600' : payModalTab === 'gov_fee' ? 'text-indigo-600' : 'text-emerald-600'} />
                  <div>
                    <h3 className="text-sm font-bold text-foreground leading-none">
                      {payModalTab === 'refund' ? '↩ Return Money / Refund' : payModalTab === 'gov_fee' ? '🏛️ Gov Fee & Service Cost' : 'Payments & Advance'} — #{payingSaleDetails.invoice_no}
                    </h3>
                    <span className="text-[10px] text-muted-foreground mt-1 block">
                      Customer: {payingSaleDetails.customer?.name || 'Walk-in Customer'} {payingSaleDetails.person_name ? `• Applicant: ${payingSaleDetails.person_name}` : ''}
                    </span>
                  </div>
                </div>

                {/* 3-WAY TAB SWITCHER: Collect Payment vs Gov Fee / Cost vs Return Money */}
                <div className="flex rounded-xl bg-muted/50 p-1 border border-border mb-4 gap-1">
                  <button
                    type="button"
                    onClick={() => {
                      setPayModalTab('collect');
                  ...
                  <button
                    type="button"
                    onClick={() => setPayModalTab('gov_fee')}
                    className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
                      payModalTab === 'gov_fee'
                  ...
                  >
                    <Landmark size={13} />
                    <span>Gov Fee / Cost</span>
                    {totalGovFeesPaid > 0 ? (
                      <span className={`text-[9px] px-1.5 py-0.2 rounded-full font-bold ${payModalTab === 'gov_fee' ? 'bg-white/20 text-white' : 'bg-indigo-100 text-indigo-700'}`}>
                        {totalGovFeesPaid.toFixed(0)} AED
                      </span>
                    ) : (
                      <span className={`text-[9px] px-1.5 py-0.2 rounded-full font-semibold ${payModalTab === 'gov_fee' ? 'bg-white/20 text-white' : 'bg-slate-200 text-slate-700'}`}>
                        Est: {defaultExpectedGovFee.toFixed(0)} AED
                      </span>
                    )}
                  </button>
                ...
                {/* 5-COLUMN FINANCIAL SUMMARY CARDS */}
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 mb-4">
                  <div className="bg-muted/30 border border-border rounded-xl p-2.5 text-center">
                    <div className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5">Invoice Total</div>
                  ...
                  <div className="bg-indigo-500/5 border border-indigo-500/20 rounded-xl p-2.5 text-center">
                    <div className="text-[9px] font-bold text-indigo-600/70 uppercase tracking-wider mb-0.5 flex items-center justify-center gap-1">
                      <span>Gov Fee Cost</span>
                      <span className="text-[8px] font-normal">{totalGovFeesPaid > 0 ? '(Paid)' : '(Est.)'}</span>
                    </div>
                    <div className="text-xs font-bold text-indigo-700">{currentGovFeeCost.toFixed(2)} <span className="text-[9px]">AED</span></div>
                  </div>

                  <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-2.5 text-center">
                    <div className="text-[9px] font-bold text-emerald-700/70 uppercase tracking-wider mb-0.5">Typing Margin</div>
                    <div className="text-xs font-bold text-emerald-700">{typingMarginProfit.toFixed(2)} <span className="text-[9px]">AED</span></div>
                  </div>

                  <div className={`col-span-2 sm:col-span-1 border rounded-xl p-2.5 text-center ${
                    advanceCredit > 0
                    ...
                    <div className={`text-[9px] font-bold uppercase tracking-wider mb-0.5 ${
                      ...
                      {advanceCredit > 0 ? '✨ Advance' : isPaid ? 'Paid' : 'Customer Due'}
                    </div>
                    <div className={`text-xs font-black ${
                      advanceCredit > 0 ? 'text-sky-800' : isPaid ? 'text-emerald-700' : 'text-rose-600'
                    }`}>
                      {advanceCredit > 0 ? `+${advanceCredit.toFixed(2)}` : isPaid ? 'Full' : `${due.toFixed(2)} AED`}
                    </div>
                  </div>
                </div>
                ...
                {payModalTab === 'collect' && (
                  <div className="space-y-4">
                    {/* Service charge summary banner */}
                    <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex flex-wrap items-center justify-between text-xs gap-2">
                      <div className="flex items-center gap-2">
                        <Tag size={14} className="text-emerald-700" />
                        <span className="font-semibold text-emerald-900">
                          Total Invoice Fee: <strong className="font-black">{grandTotal.toFixed(2)} AED</strong>
                        </span>
                      </div>
                      <div className="flex items-center gap-3 text-[11px] text-emerald-800">
                        <span>Gov Cost: <strong>{currentGovFeeCost.toFixed(2)} AED</strong></span>
                        <span>•</span>
                        <span>Our Profit: <strong className="text-emerald-700">{typingMarginProfit.toFixed(2)} AED</strong></span>
                      </div>
                    </div>
                    ...
                    <div className="border border-border rounded-xl overflow-hidden">
                      <div className="bg-muted/40 px-3 py-1.5 text-[10px] font-bold text-muted-foreground uppercase tracking-wider border-b border-border flex justify-between items-center">
                        ...
                        <span className="text-[9px] text-muted-foreground">Net Collected: {netPaid.toFixed(2)} AED</span>
                      </div>
                      {payments.length === 0 ? (
                          ...
                          <table className="w-full text-xs">
                            <thead className="bg-muted/20 sticky top-0">
                              <tr>
                                ...
                                <th className="px-2.5 py-1 text-left text-muted-foreground font-semibold">Mode &amp; Deposit To</th>
                    ...
                    <form onSubmit={handleSubmitPayment} className="border border-emerald-500/20 bg-emerald-500/5 rounded-xl p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <p className="text-[11px] font-bold text-emerald-800 uppercase tracking-wider">
                          {isPaid ? '+ Record Advance / Additional Payment' : 'Collect Customer Payment'}
                        </p>
                        {isPaid && (
                ...
                {/* TAB 2: GOV FEE & SERVICE COST */}
                {payModalTab === 'gov_fee' && (
                  <div className="space-y-4">
                    {/* Services Cost & Profit Table */}
                    <div className="border border-border rounded-xl overflow-hidden">
                      <div className="bg-indigo-500/10 px-3 py-2 text-[10px] font-bold text-indigo-900 uppercase tracking-wider border-b border-indigo-500/20 flex justify-between items-center">
                        <span className="flex items-center gap-1.5">
                          <Landmark size={12} className="text-indigo-700" />
                          Invoice Services, Gov Fees &amp; Profit Margins
                        </span>
                        <span className="text-indigo-800">
                          Total Gov Paid: <strong>{totalGovFeesPaid.toFixed(2)} AED</strong>
                        </span>
                      </div>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-xs">
                          <thead className="bg-muted/30">
                            <tr>
                              <th className="px-3 py-1.5 text-left font-semibold text-muted-foreground">Service Name</th>
                              <th className="px-2 py-1.5 text-center font-semibold text-muted-foreground">Qty</th>
                              <th className="px-2 py-1.5 text-right font-semibold text-muted-foreground">Selling Price</th>
                              <th className="px-2 py-1.5 text-right font-semibold text-muted-foreground">Default Gov Cost</th>
                              <th className="px-2 py-1.5 text-right font-semibold text-muted-foreground">Gov Fee Paid</th>
                              <th className="px-2 py-1.5 text-right font-semibold text-muted-foreground">Typing Profit</th>
                              <th className="px-2 py-1.5 text-center font-semibold text-muted-foreground">Action</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-border/40">
                            {items.map((item: any) => {
                              const srv = item.service;
                              const selling = Number(item.subtotal) || (Number(item.unit_price) * (Number(item.quantity) || 1));
                              const defExp = (Number(srv?.expense) || 0) * (Number(item.quantity) || 1);
                              const paidExp = Number(item.expense) || 0;
                              const currentCost = paidExp > 0 ? paidExp : defExp;
                              const profit = selling - currentCost;

                              return (
                                <tr key={item.id} className="hover:bg-muted/10">
                                  <td className="px-3 py-2">
                                    <div className="font-bold text-foreground text-xs leading-tight">{srv?.name || 'Service'}</div>
                                    {item.person_name && (
                                      <span className="inline-flex items-center gap-0.5 px-1 py-0.2 rounded bg-primary/10 text-primary text-[9px] font-semibold mt-0.5">
                                        👤 {item.person_name}
                                      </span>
                                    )}
                                  </td>
                                  <td className="px-2 py-2 text-center text-muted-foreground font-semibold">
                                    {item.quantity}
                                  </td>
                                  <td className="px-2 py-2 text-right font-bold text-foreground">
                                    {selling.toFixed(2)} AED
                                  </td>
                                  <td className="px-2 py-2 text-right text-muted-foreground">
                                    {defExp.toFixed(2)} AED
                                  </td>
                                  <td className="px-2 py-2 text-right">
                                    {paidExp > 0 ? (
                                      <span className="font-bold text-indigo-700 bg-indigo-50 border border-indigo-200 px-1.5 py-0.5 rounded text-[11px]">
                                        ✓ {paidExp.toFixed(2)} AED
                                      </span>
                                    ) : (
                                      <span className="text-amber-700 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded text-[10px] font-semibold">
                                        Unpaid (Est: {defExp.toFixed(0)})
                                      </span>
                                    )}
                                  </td>
                                  ...
                                  <td className="px-2 py-2 text-center">
                                    <button
                                      type="button"
                                      onClick={() => {
                                        setPayGovItemId(item.id);
                                        setPayGovAmount(paidExp > 0 ? 0 : Number(srv?.expense || 0));
                                        setPayGovDesc(`${srv?.name || 'Service'} Gov Fee${item.person_name ? ` (${item.person_name})` : ''}`);
                                      }}
                                      className="px-2 py-0.5 rounded bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 hover:border-indigo-300 text-[10px] font-bold transition-all cursor-pointer inline-flex items-center gap-1"
                                    >
                                      <Plus size={10} />
                                      {paidExp > 0 ? 'Add Extra' : 'Log Fee'}
                                    </button>
                                  </td>
                                </tr>
                              );
                    ...
                    {/* Log Gov Fee Expense Form */}
                    <form onSubmit={handleLogGovFeeFromPayModal} className="border border-indigo-500/20 bg-indigo-500/5 rounded-xl p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <p className="text-[11px] font-bold text-indigo-900 uppercase tracking-wider flex items-center gap-1.5">
                          ...
                          Record Government Fee / Card Expense (Deducts Account &amp; Saves to Expenses)
                        </p>
                        <span className="text-[10px] font-bold px-2 py-0.5 bg-indigo-100 text-indigo-900 rounded-md border border-indigo-200">
                          Auto-linked to #{payingSaleDetails.invoice_no}
                        </span>
                      </div>

                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div className="space-y-1">
                          <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">For Service Item *</label>
                          <select
                            value={payGovItemId}
                            onChange={(e) => {
                        ...
                        <div className="space-y-1">
                          <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Gov Fee Amount (AED) *</label>
                          <input
                            type="number"
                            min={0.01}
                            step={0.01}
                            value={payGovAmount || ''}
                            onChange={(e) => setPayGovAmount(parseFloat(e.target.value) || 0)}
                            placeholder="0.00"
                            className="w-full px-3 py-1.5 rounded-lg border border-border bg-card text-xs font-bold text-foreground focus:ring-1 focus:ring-indigo-500"
                            required
                          />
                        </div>

                        <div className="space-y-1">
                          <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Paid From Card / Account *</label>
                          <select
                            value={payGovAccountId}
                            onChange={(e) => setPayGovAccountId(e.target.value)}
                            required
                            className="w-full px-3 py-1.5 rounded-lg border border-border bg-card text-xs font-bold text-foreground focus:ring-1 focus:ring-indigo-500 cursor-pointer"
                          >
                            <option value="">-- Select Corporate Card / Account * --</option>
                            {accounts.map(a => (
                              <option key={a.id} value={a.id}>
                                {a.type === 'card' ? '💳' : a.type === 'bank' ? '🏦' : '💵'} {a.name} ({a.balance.toFixed(2)} AED)
                              </option>
                            ))}
                          </select>
                        </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="space-y-1">
                          <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Expense Date *</label>
                          <input
                            type="date"
                            value={payGovDate}
                            onChange={(e) => setPayGovDate(e.target.value)}
                            className="w-full px-3 py-1.5 rounded-lg border border-border bg-card text-xs font-medium text-foreground focus:ring-1 focus:ring-indigo-500"
                            required
                          />
                        </div>
                        <div className="space-y-1">
                          <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Description / Note</label>
                          <input
                            type="text"
                            value={payGovDesc}
                            onChange={(e) => setPayGovDesc(e.target.value)}
                            placeholder="E.g. Tawjeeh Gov Fee, Medical Typing Fee..."
                            className="w-full px-3 py-1.5 rounded-lg border border-border bg-card text-xs font-medium text-foreground focus:ring-1 focus:ring-indigo-500"
                          />
                        </div>
                      </div>
                      ...
                      <div className="flex justify-end pt-1">
                        <button
                          type="submit"
                          disabled={payGovSaving || payGovAmount <= 0 || !payGovAccountId}
                          className="flex items-center gap-1.5 px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl transition-all shadow-md disabled:opacity-50 cursor-pointer"
                        >
                          <Landmark size={13} />
                          {payGovSaving ? 'Recording Expense...' : `Record ${payGovAmount.toFixed(2)} AED Gov Fee Expense`}
                        </button>
                      </div>
                    </form>

                    {/* Logged Gov Fee Transactions */}
                    {allItemExpenses.length > 0 && (
                      <div className="border border-border rounded-xl overflow-hidden">
                        <div className="bg-muted/40 px-3 py-1.5 text-[10px] font-bold text-muted-foreground uppercase tracking-wider border-b border-border flex justify-between items-center">
                          <span>Logged Gov Fees &amp; Expenses for this Invoice ({allItemExpenses.length})</span>
                          <span className="text-indigo-700 font-bold">Total: {totalGovFeesPaid.toFixed(2)} AED</span>
                        </div>
                        <div className="max-h-36 overflow-y-auto">
                          <table className="w-full text-xs">
                            <thead className="bg-muted/20 sticky top-0">
                              <tr>
                                <th className="px-3 py-1 text-left text-muted-foreground font-semibold">Date</th>
                                <th className="px-3 py-1 text-left text-muted-foreground font-semibold">Service / Description</th>
                                <th className="px-3 py-1 text-left text-muted-foreground font-semibold">Paid From Card</th>
                                <th className="px-3 py-1 text-right text-muted-foreground font-semibold">Amount</th>
                                <th className="px-3 py-1 text-center text-muted-foreground font-semibold">Action</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-border/40">
                              {allItemExpenses.map((exp: any, idx: number) => {
                                const acc = accounts.find(a => a.id === exp.account_id);
                                return (
                                  <tr key={exp.id || idx}>
                                    <td className="px-3 py-1 text-muted-foreground font-mono text-[10px]">
                                      {exp.expense_date || '—'}
                                    </td>
                                    <td className="px-3 py-1">
                                      <div className="font-bold text-foreground text-xs leading-tight">
                                        {exp.description || exp.itemService || 'Gov Fee'}
                                      </div>
                                      {exp.itemPerson && (
                                        <span className="text-[9px] text-muted-foreground">👤 {exp.itemPerson}</span>
                                      )}
                                    </td>
                                    <td className="px-3 py-1">
                                      {acc ? (
                                        ...
                                        <span className="text-[10px] text-muted-foreground">{exp.payment_method || 'Card'}</span>
                                      )}
                                    </td>
                                    <td className="px-3 py-1 text-right font-black text-rose-700">
                                      -{Number(exp.amount || 0).toFixed(2)} AED
                                    </td>
                                    <td className="px-3 py-1 text-center">
                                      <button
                                        type="button"
                                        onClick={() => handleDeleteGovFeeFromPayModal(exp.id)}
                                        disabled={payGovSaving}
                                        className="p-1 text-rose-500 hover:text-rose-700 hover:bg-rose-50 rounded transition-colors cursor-pointer"
                                        title="Delete Gov Fee Expense (Restores Card Balance)"
                                      >
                                        <Trash2 size={12} />
                                      </button>
                                    </td>
        ...
        {/* UNIVERSAL TRANSACTION CONFIRMATION MODAL */}
        {salesConfirmModal && (
          <TransactionConfirmModal
            isOpen={salesConfirmModal.isOpen}
            onClose={() => setSalesConfirmModal(null)}
            onConfirm={salesConfirmModal.onConfirm}
            type={salesConfirmModal.type}
            title={salesConfirmModal.title}
            subtitle={salesConfirmModal.subtitle}
            amount={salesConfirmModal.amount}
            currency="AED"
            confirmText={salesConfirmModal.confirmText}
            cancelText="Back to Edit"
            warningMessage={salesConfirmModal.warningMessage}
            loading={paySaving || payGovSaving}
            details={salesConfirmModal.details}
          />
        )}
